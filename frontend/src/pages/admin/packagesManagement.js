  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-black dark:text-white">Packages</h1>
          <p className="text-black/70 dark:text-white/70">Create and manage your event packages and services</p>
        </div>
        <button
          className="inline-flex items-center px-6 py-3 bg-gradient-to-r from-amber-500 to-amber-600 text-black font-semibold rounded-xl hover:from-amber-600 hover:to-amber-700 transition-all duration-200 transform hover:scale-105 shadow-lg mt-4 md:mt-0"
      ...
      {/* Filters */}
      <div className="bg-white/80 dark:bg-black/70 backdrop-blur-sm rounded-2xl border border-black/10 dark:border-white/10 shadow-lg overflow-hidden">
        <div className="px-6 py-4 border-b border-black/10 dark:border-white/10 bg-gradient-to-r from-white/80 to-white/60 dark:from-black/60 dark:to-black/40">
          <h3 className="text-sm font-semibold text-black dark:text-white">Filter & Search</h3>
        </div>
        <div className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="relative">
              <input
                type="text"
                className="w-full pl-4 pr-4 py-3 border-0 rounded-xl bg-white/60 dark:bg-black/60 text-black dark:text-white placeholder-black/50 dark:placeholder-white/50 focus:outline-none focus:ring-2 focus:ring-amber-500/40 focus:bg-white dark:focus:bg-black transition-all duration-200"
            ...
            <select
              className="w-full px-4 py-3 border-0 rounded-xl bg-white/60 dark:bg-black/60 text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-amber-500/40 focus:bg-white dark:focus:bg-black transition-all duration-200"
            ...
            <label className="inline-flex items-center space-x-3 px-4 py-3 bg-white/60 dark:bg-black/60 rounded-xl cursor-pointer hover:bg-white/80 dark:hover:bg-black/80 transition-all duration-200">
              <input type="checkbox" className="h-4 w-4 text-amber-600 rounded" checked={onlyPopular} onChange={(e) => setOnlyPopular(e.target.checked)} />
              <span className="text-sm font-medium text-black dark:text-white">Popular only</span>
            </label>
            <label className="inline-flex items-center space-x-3 px-4 py-3 bg-white/60 dark:bg-black/60 rounded-xl cursor-pointer hover:bg-white/80 dark:hover:bg-black/80 transition-all duration-200">
              <input type="checkbox" className="h-4 w-4 text-amber-600 rounded" checked={onlyFeatured} onChange={(e) => setOnlyFeatured(e.target.checked)} />
              <span className="text-sm font-medium text-black dark:text-white">Featured only</span>
            </label>
          </div>
        </div>
      </div>
      ...
      {/* Packages */}
      {viewList.length === 0 ? (
        <div className="flex flex-col items-center justify-center text-center bg-white/80 dark:bg-black/70 border border-black/10 dark:border-white/10 rounded-2xl p-10 shadow-lg">
          <svg className="w-10 h-10 text-black/40 dark:text-white/40 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />
          </svg>
          <h3 className="text-lg font-semibold text-black dark:text-white mb-2">No packages found</h3>
          <p className="text-black/60 dark:text-white/60 mb-6">Get started by creating your first package</p>
          <button
            className="inline-flex items-center px-6 py-3 bg-gradient-to-r from-amber-500 to-amber-600 text-black font-semibold rounded-xl hover:from-amber-600 hover:to-amber-700 transition-all duration-200 transform hover:scale-105 shadow-lg"
            onClick={() => alert('Add Package functionality coming soon')}
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {viewList.map((pkg) => (
            <div key={pkg._id} className="bg-white/80 dark:bg-black/70 backdrop-blur-sm border border-black/10 dark:border-white/10 rounded-2xl overflow-hidden hover:shadow-xl transition-all duration-200 transform hover:scale-105">
              {pkg.imageUrl && (
                <div className="w-full h-48 bg-gradient-to-br from-white/60 to-white/40 dark:from-black/60 dark:to-black/40 overflow-hidden relative">
                  <img src={pkg.imageUrl} alt={pkg.name} className="w-full h-full object-cover" />
                  <div className="absolute top-3 right-3">
                    <div className="px-3 py-1 bg-white/90 dark:bg-black/90 backdrop-blur-sm rounded-full text-sm font-bold text-black dark:text-white">
              ...
              <div className="p-6">
                <div className="flex justify-between items-center mb-4">
                  <h2 className="text-xl font-bold text-black dark:text-white">{pkg.name}</h2>
                  <button
                    onClick={() => { setEditPackage(pkg); setShowEditModal(true); }}
                    className="inline-flex items-center px-3 py-1.5 rounded-lg bg-gradient-to-r from-amber-500 to-amber-600 text-black font-semibold hover:from-amber-600 hover:to-amber-700"
                  >
                    Edit
                  </button>
                </div>
                <p className="text-sm text-black/70 dark:text-white/70 mb-3">{pkg.description}</p>
                <div className="flex flex-wrap gap-2 mb-4">
                  {Array.isArray(pkg.features) && pkg.features.map((f, i) => (
                    <span key={i} className="bg-amber-100 text-amber-800 text-xs font-medium px-2.5 py-0.5 rounded">{f}</span>
                  ))}
                </div>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    {pkg.isPopular && <span className="px-2 py-0.5 rounded text-xs bg-amber-100 text-amber-800">Popular</span>}
                    {pkg.isFeatured && <span className="px-2 py-0.5 rounded text-xs bg-amber-200 text-amber-900">Featured</span>}
                  </div>
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => handleDelete(pkg._id)}
                      className="px-3 py-1.5 rounded-lg border border-black/20 text-black dark:text-white hover:bg-black/5 dark:hover:bg-white/10"
                    >
                      Delete
                    </button>
                  </div>
